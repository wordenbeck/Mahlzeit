...
function parseMarkdownRecipes(content: string) {
  ...
  for (const block of recipeBlocks) {
    ...
    if (!titel || titel.includes('Batch') || titel.includes('Extraktion') || seenTitels.has(titel.toLowerCase())) {
      continue;
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (zeitMatch) zubereitungszeit_min = parseInt(zeitMatch[1]);
      ...
      if (line.match(/^---/) || line.match(/^[#]{1,2}\s+/)) {
        if (inZutaten || inZubereitung) break;
      }
    ...
    if (titel && zutaten.length > 0 && zubereitung) {
      recipes.push({ titel, zutaten, zubereitung, zubereitungszeit_min });
    }
  }
  return recipes;
}
...
for (const file of ['rezepte_batch_1.md', 'Rezepte_Batch_2.md', 'rezepte_batch_3.md']) {
  ...
  if (fs.existsSync(filePath)) {
    allRecipes.push(...parseMarkdownRecipes(fs.readFileSync(filePath, 'utf-8')));
  }
}
...
for (const r of allRecipes) {
  ...
  const zeit = r.zubereitungszeit_min || 'NULL';

  console.log(`INSERT INTO recipes (titel, zutaten, zubereitung, zubereitungszeit_min, schwierigkeit, is_shared, created_at, updated_at) VALUES ('${titel}', '${zutatenJson}'::jsonb, '${zubereitung}', ${zeit}, 'medium', true, NOW(), NOW());`);
}
